...
class GnnNet(MetaTemplate):
    ...
    def __init__(self, model_func, n_way, n_support):
        ...
        self.fc = nn.Sequential(nn.Linear(self.feat_dim, 128),
                                nn.BatchNorm1d(128, track_running_stats=False)) if not self.FWT else nn.Sequential(
    ...
    def set_forward(self, x, is_feature=False):
        ...
        if is_feature:
            ...
            # x: (105,3,224,224)
            x = x.view(-1, *x.size()[2:])
            # self.feature(x):([105,512]) 为通过resnet提取到的特征表示
            z = self.fc(self.feature(x))
            z = z.view(self.n_way, -1, z.size(1))
        # z.size():[5, 21, 128];  z[:, :self.n_support].size():torch.Size([5, 5, 128]);
        # z[:, self.n_support + i:self.n_support + i + 1]].size():[5, 1, 128] concat:[5,6,128] 相当于6 support set
        # [25 support set + 属于5类别的query]
    ...
    def forward_gnn(self, zs):
        # fix label for training the metric function   1*nw(1 + ns)*nw ([25,1])
        support_label = torch.from_numpy(np.repeat(range(self.n_way), self.n_support)).unsqueeze(1)
        support_label = torch.zeros(self.n_way * self.n_support, self.n_way).scatter(1, support_label, 1).view(
            self.n_way, self.n_support, self.n_way) #([5,5,5])
        support_label = torch.cat([support_label, torch.zeros(self.n_way, 1, self.n_way)], dim=1) # 其它query的级联标签都为0 相当于生成了16张子图,每张子图包含5个级联全0标签的query样本
        support_label = support_label.view(1, -1, self.n_way).cuda() #每张子图包含5个各类一个的待分类query
        # gnn inp: n_q * n_way(n_s + 1) * f     z.size():([1,30,128]) nodes.size() ([16,30,133])
        nodes = torch.cat([torch.cat([z, support_label], dim=2) for z in zs], dim=0)
        scores = self.gnn(nodes) # scores: [16, 30, 5]
...
class GnnNetLRP(GnnNet):
    ...
    def _get_gnnfeature_relevance(self, gnn_nodes):
        W_init = torch.eye(gnn_nodes.size(1), device=gnn_nodes.device).unsqueeze(0).repeat(gnn_nodes.size(0), 1,
                                                                                           1).unsqueeze(
            3)  # (n_querry, n_way*(num_support + 1), n_way*(num_support + 1), 1)
        # the first iteration
        W1 = self.gnn._modules['layer_w{}'.format(0)](gnn_nodes,
                                                      W_init)  # (n_querry, n_way*(num_support + 1), n_way*(num_support + 1), 2)
        x_new1 = F.leaky_relu(self.gnn._modules['layer_l{}'.format(0)]([W1, gnn_nodes])[
                                  1])  # (num_querry, n_way*(num_support + 1), num_outputs)
        gnn_nodes_1 = torch.cat([gnn_nodes, x_new1], 2)  # (concat more features)

        #  the second iteration
        W2 = self.gnn._modules['layer_w{}'.format(1)](gnn_nodes_1,
                                                      W_init)  # (n_querry, n_way*(num_support + 1), n_way*(num_support + 1), 2)
        x_new2 = F.leaky_relu(self.gnn._modules['layer_l{}'.format(1)]([W2, gnn_nodes_1])[
                                  1])  # (num_querry, n_way*(num_support + 1), num_outputs)
        ...
        gnn_logits = torch.log(LRPutil.LOGIT_BETA * (scores_sf + LRPutil.EPSILON) / (
                    torch.tensor([1 + LRPutil.EPSILON]).cuda() - scores_sf))
        ...
        relevance_gnn_nodes_1 = self.explain_Gconv(relevance_x_new2, self.gnn._modules['layer_l{}'.format(1)], W2,
                                                   gnn_nodes_1)
        relevance_x_new1 = relevance_gnn_nodes_1.narrow(-1, 133, 48)
        relevance_gnn_nodes = self.explain_Gconv(relevance_x_new1, self.gnn._modules['layer_l{}'.format(0)], W1,
                                                 gnn_nodes)
    ...
    def forward_gnn(self, zs):
        ...
        if self.lrptraining:
            ...
            nodes = torch.cat([torch.cat([zs[i] * relevance_weight[i], support_label], dim=2) for i in range(bs)],
                              dim=0)
            scores_lrp = self.gnn(nodes)
            scores_lrp = scores_lrp.view(self.n_query, self.n_way, self.n_support + 1, self.n_way)[:, :, -1].permute(1,
                                                                                                                     0,
                                                                                                                     2).contiguous().view(
                -1, self.n_way)
