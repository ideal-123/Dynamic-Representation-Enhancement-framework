...
def gmul(input):
  ...
  N = W_size[-2]
  W = W.split(1, 3)
  W = torch.cat(W, 1).squeeze(3) # W is now a tensor of size (bs, J*N, N) #W: (16,60,30) x:[16,30,133]
  output = torch.bmm(W, x) # output has size (bs, J*N, num_features) bmm:batch matrix multiplication output:[16, 60, 133]
  output = output.split(N, 1)# 沿着dim=1进行30个维度一次的切分得到两个[16,30,133]
  output = torch.cat(output, 2) # output has size (bs, N, J*num_features) 关于两个邻接矩阵得到共266维新特征
  return output
...
class Gconv(nn.Module):
  ...
  def forward(self, input):
    W = input[0] # input[0]为两个邻接矩阵的concate
    x = gmul(input) # out has size (bs, N, num_inputs)
    #if self.J == 1:
    #    x = torch.abs(x)
    x_size = x.size()
    x = x.contiguous()
    x = x.view(-1, self.num_inputs) #([480,362])
    x = self.fc(x) # has size (bs*N, num_outputs)

    if self.bn_bool:
      x = self.bn(x)
    x = x.view(*x_size[:-1], self.num_outputs) #([16,30,48])
    return W, x
...
class Wcompute(nn.Module):
  ...
  def forward(self, x, W_id): #x:[16, 30, 133];
    W1 = x.unsqueeze(2) #x:[16, 30, 1, 133]
    W2 = torch.transpose(W1, 1, 2) #size: bs x N x N x num_features [16, 1, 30, 133]
    W_new = torch.abs(W1 - W2) #size: bs x N x N x num_features W_new用于求邻接矩阵
    W_new = torch.transpose(W_new, 1, 3) #size: bs x num_features x N x N   --- 求出新的邻接矩阵 W_new:[16, 133, 30, 30]

    W_new = self.conv2d_1(W_new)
    W_new = self.bn_1(W_new)
    W_new = F.leaky_relu(W_new) # [16, 192, 30, 30]
    if self.drop:
    ...
    W_new = self.conv2d_last(W_new) # 输出通道为1
    W_new = torch.transpose(W_new, 1, 3) #size: bs x N x N x1  [16, 1, 30, 30]
    ...
    if self.activation == 'softmax':
      ...
      # Softmax applied
      W_new = torch.transpose(W_new, 2, 3) # softmax后新的邻接矩阵
    ...
    return W_new # ([16,30,30,2]) 后面的2可以理解为是不同层的邻接矩阵的加和 即不同邻接算子在x上作用后的加和
...
class GNN_nl(nn.Module):
  ...
  def forward(self, x): # x.size():[16,30,133]
    W_init = torch.eye(x.size(1), device=x.device).unsqueeze(0).repeat(x.size(0), 1, 1).unsqueeze(3) # W_init:([16,30,30,1])

    for i in range(self.num_layers): # self._modules['layer_w{}'.format(i)]:Wcompute
      Wi = self._modules['layer_w{}'.format(i)](x, W_init) #layer_w0 只和W_init concate
                                     # Gconv
