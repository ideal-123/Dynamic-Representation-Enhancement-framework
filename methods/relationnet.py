...
from LRPtools import lrp_wrapper, lrp_presets, utils as LRPutil
import copy
import gc
import utils
...
class RelationNet(MetaTemplate):
    ...
    def set_forward(self, x, is_feature=False, supp_query=False):
        ...
        if supp_query:
            ...
            # relation_module中包含神经网络里需要训练的参数
            relations = self.relation_module(relation_pairs).view(-1, self.n_way)
            # metric-based method之间的不同在于对得到的特征向量进行不同的处理后再计算损失
            return relations

    def set_forward_loss(self, x):
        y = torch.from_numpy(np.repeat(range(self.n_way), self.n_query))
        # scores为得到的用于和真实标签做mse等loss计算的向量
    ...
    def scores_calculate(self, z_support, z_query):
        z_proto = z_support.view(self.n_way, self.n_support, *self.feat_dim).mean(1)
        z_query = z_support.view(z_query.size(0), *self.feat_dim)
        # z_proto_ext = z_proto.unsqueeze(0).repeat(self.n_query * self.n_way, 1, 1, 1, 1)
        z_proto_ext = z_proto.unsqueeze(0).repeat(z_query.size(0), 1, 1, 1, 1)
        z_query_ext = z_query.unsqueeze(0).repeat(self.n_way, 1, 1, 1, 1)
        z_query_ext = torch.transpose(z_query_ext, 0, 1)
        extend_final_feat_dim = self.feat_dim.copy()
        extend_final_feat_dim[0] *= 2
        relation_pairs = torch.cat((z_proto_ext, z_query_ext), 2).view(-1, *extend_final_feat_dim)
        # relation_module中包含神经网络里需要训练的参数
        scores = self.relation_module(relation_pairs).view(-1, self.n_way)
        return scores
...
class RelationNetLRP(RelationNet):
    ...
    def get_feature_relevance(self, relation_pairs, relations):
        ...
        relations_logits = torch.log(LRPutil.LOGIT_BETA * (relations_sf + LRPutil.EPSILON) / (
                    torch.tensor([1 + LRPutil.EPSILON]).cuda() - relations_sf))
        ...
        relevance_relations = model.compute_lrp(relation_pairs,
                                                target=relations_logits)  # (n_way*n_querry * n_support, 2* feature_dim, f_h, f_w)
