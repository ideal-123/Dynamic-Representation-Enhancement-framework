...
def train(base_loader, val_loader, model, start_epoch, stop_epoch, params, writer):
    ...
    for epoch in range(start_epoch, stop_epoch):
        model.train()
        total_it, writer = model.train_loop(epoch, base_loader, optimizer, total_it, params, epoch_num, writer)
...
if __name__ == '__main__':
    ...
    params = parse_args()
    params.rsc=True
    params.lifted_struct_loss=True
    params.model='ResNet10_mask'
    ...
    if params.lifted_struct_loss:
        if params.s1_rate != 1.0:
            model.loss_fn = Lifted_Struct_loss(s1_rate=params.s1_rate)
        else:
